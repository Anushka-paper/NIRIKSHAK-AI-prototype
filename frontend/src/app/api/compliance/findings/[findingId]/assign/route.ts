...
export async function POST(request: Request, context: { params: Promise<{ findingId: string }> }) {
  try {
    ...
    const res = await fetch(`${BACKEND_API}/findings/${findingId}/assign`, {
      method: "POST",
    ...
    if (!res.ok) {
      return NextResponse.json({ success: false, error: data.detail || "Failed to assign finding" }, { status: res.status });
    }
