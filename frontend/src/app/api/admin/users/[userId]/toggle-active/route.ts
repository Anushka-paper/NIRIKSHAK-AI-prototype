...
export async function POST(request: Request, context: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const res = await fetch(`${BACKEND_API}/admin/users/${userId}/toggle-active`, {
      method: "POST",
      headers: forwardAuthHeader(request),
    });
    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json({ success: false, error: data.detail || "Failed to update user" }, { status: res.status });
    }
