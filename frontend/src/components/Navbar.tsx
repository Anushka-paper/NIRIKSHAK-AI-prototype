...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-surface/80 backdrop-blur">
      <div className="container mx-auto flex h-16 items-center px-4 md:px-8">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg p-2 text-gray-600 hover:bg-slate-100 md:hidden"
          >
      ...
      {mobileOpen && (
        <nav className="border-t bg-white px-4 py-2 font-body font-semibold md:hidden">
          {allLinks.map((link) => {
            const isActive = pathname === link.href;
            return (
              <Link
                ...
                className={`block rounded-lg px-2 py-3 text-sm transition-colors ${
                  isActive ? "text-primary bg-orange-50" : "text-gray-600 hover:bg-slate-50 hover:text-primary"
                }`}
              >
                {link.name}
              </Link>
